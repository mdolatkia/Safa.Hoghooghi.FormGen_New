...
namespace MyModelManager
{
    public class BizEntitySettings
    {
        ...
        public void UpdateDefaultSettingsInModel(DR_Requester requester, int databaseID)
        {
            ...
            var searchEntities = bizTableDrivedEntity.GetOrginalEntitiesWithoutEntitySearch(databaseID);
            var initialSearchEntities = bizTableDrivedEntity.GetOrginalEntitiesWithoutInitiallySearch(databaseID);
            ...
            , List<TableDrivedEntityDTO> allEntities)
        {
            ...
            using (var projectContext = new DataAccess.MyIdeaEntities())
            {
                ...
                foreach (var entityID in uiCompositionEntities)
                {

                    if (ItemImportingStarted != null)
                        ItemImportingStarted(this, new ItemImportingStartedArg() { ItemName = "Fetching entity" + " " + GetEntityInfo(allEntities, entityID), TotalProgressCount = uiCompositionEntities.Count(), CurrentProgress = uiCompositionEntities.IndexOf(entityID) + 1 });
                    var uiComposition = bizEntityUIComposition.GetOrCreateEntityUIComposition(entityID);
                    listEntityUIComposition.Add(new Tuple<int, EntityUICompositionDTO>(entityID, uiComposition));
                }
                ...
                foreach (var entityID in listViewEntities)
                {
                    if (ItemImportingStarted != null)
                        ItemImportingStarted(this, new ItemImportingStartedArg() { ItemName = "Fetching entity" + " " + GetEntityInfo(allEntities, entityID), TotalProgressCount = listViewEntities.Count(), CurrentProgress = listViewEntities.IndexOf(entityID) + 1 });
                    var viewItem = bizEntityListView.GetOrCreateEntityListViewDTO(entityID);
                    listEntityAndView.Add(new Tuple<int, EntityListViewDTO>(entityID, viewItem));
                }
                ...
                {
                    if (ItemImportingStarted != null)
                        ItemImportingStarted(this, new ItemImportingStartedArg() { ItemName = "Setting default view for entity" + " " + GetEntityInfo(allEntities, item.Item1), TotalProgressCount = listEntityAndView.Count(), CurrentProgress = listEntityAndView.IndexOf(item) + 1 });
                ...
                List<Tuple<int, EntitySearchDTO>> listEntityAndSearch = new List<Tuple<int, EntitySearchDTO>>();
                foreach (var entityID in searchEntities)
                {
                    if (ItemImportingStarted != null)
                        ItemImportingStarted(this, new ItemImportingStartedArg() { ItemName = "Fetching entity" + " " + GetEntityInfo(allEntities, entityID), TotalProgressCount = searchEntities.Count(), CurrentProgress = searchEntities.IndexOf(entityID) + 1 });
                    var searchItem = bizEntitySearch.GetOrCreateEntitySearchDTO(entityID);
                    listEntityAndSearch.Add(new Tuple<int, EntitySearchDTO>(entityID, searchItem));
                }
                foreach (var item in listEntityAndSearch)
                {
                    if (ItemImportingStarted != null)
                        ItemImportingStarted(this, new ItemImportingStartedArg() { ItemName = "Setting default search for entity" + " " + GetEntityInfo(allEntities, item.Item1), TotalProgressCount = listEntityAndSearch.Count(), CurrentProgress = listEntityAndSearch.IndexOf(item) + 1 });
                    var dbSearch = bizEntitySearch.SaveItem(projectContext, item.Item2, createdRelationshipTails);
                    //var dbentity = bizTableDrivedEntity.GetAllEntities(projectContext, false).First(x => x.ID == item.Item1);
                    //dbentity.EntitySearch = dbSearch;
                }


                List<Tuple<int, bool>> listEntityAndInitialSearch = new List<Tuple<int, bool>>();
                foreach (var entityID in initialSearchEntities)
                {
                    if (ItemImportingStarted != null)
                        ItemImportingStarted(this, new ItemImportingStartedArg() { ItemName = "Fetching entity" + " " + GetEntityInfo(allEntities, entityID), TotalProgressCount = searchEntities.Count(), CurrentProgress = initialSearchEntities.IndexOf(entityID) + 1 });
                    bool initiallySearched = bizTableDrivedEntity.DecideEntityIsInitialySearched(entityID);
                    listEntityAndInitialSearch.Add(new Tuple<int, bool>(entityID, initiallySearched));
                }
                foreach (var item in listEntityAndInitialSearch)
                {
                    if (ItemImportingStarted != null)
                        ItemImportingStarted(this, new ItemImportingStartedArg() { ItemName = "Setting initially searched entity" + " " + GetEntityInfo(allEntities, item.Item1), TotalProgressCount = listEntityAndInitialSearch.Count(), CurrentProgress = listEntityAndInitialSearch.IndexOf(item) + 1 });
                    bizTableDrivedEntity.UpdateEntityInitiallySearch(projectContext, item.Item1, item.Item2);
                }
        ...
        private string GetEntityInfo(List<TableDrivedEntityDTO> allEntities, int entityID)
        {
            var entity = allEntities.FirstOrDefault(x => x.ID == entityID);
            if (entity != null)
                return entity.Name;
            else
